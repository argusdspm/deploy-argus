...
> **PREVIEW STATUS** — This module works against `api.argusdspm.com` but is not yet wired into the Argus dashboard's Deploy Agent flow. Variable interface, defaults, and outputs may change in minor breaking ways before the `v1.x` stable line. Pin `?ref=v1.0` in your `source` to insulate yourself from changes.
...
```hcl
module "argus_azure" {
  source = "github.com/argusdspm/deploy-argus//modules/argus-agent-azure-aci?ref=v1.0"
