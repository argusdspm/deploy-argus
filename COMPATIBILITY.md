...
| `v0.9.1` | `ghcr.io/argusdspm/argus-agent:stable` (or `:v0.9.6`) | `https://api.argusdspm.com` | Adds stable-channel auto-update: a Fargate weekly refresh scheduler (opt-in via `agent_image_tag = "stable"`) and `ARGUS_UPDATE_CHANNEL` passthrough on both AWS modules. Re-vendors the permission registry to 0.9.6 (no new module grants - the modules already covered it; the vendored copy was stale). Non-breaking: upgrade is `terraform init -upgrade && terraform apply`; a `:stable` deploy gains the weekly-refresh resources, a pinned deploy is unchanged. |
